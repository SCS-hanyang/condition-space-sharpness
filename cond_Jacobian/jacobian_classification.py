...
def main():
    ...
    # Update Output Dir
    OUTPUT_DIR = os.path.join(BASE_DIR, "results", f"jacobian_classification_proj_{NUM_PROJECTIONS}")
    ...
    elif os.path.exists(os.path.join(BASE_DIR, "results", "jacobian_classification", "init_noises.pt")):
         # Fallback to shared init noises if not specific
        ...
        for group_name, prompts in groups:
            ...
            for p_idx, prompt in enumerate(tqdm(prompts, desc=group_name)):
                ...
                for b in range(num_batches):
                    ...
                    # Note: Scale model input usually does nothing at t=T but good practice
    ...
    print(f"Memorized -> Classified as Unmemorized (Strict): {count_mem_misclassified} / {n_mem_prompts} ({count_mem_misclassified/n_mem_prompts*100:.2f}%)")
    print(f"Unmemorized -> Classified as Memorized (Strict): {count_unmem_misclassified} / {n_unmem_prompts} ({count_unmem_misclassified/n_unmem_prompts*100:.2f}%)")
    ...
    if len(df_misclassified) > 0:
        ...
        # If mean > thresh, severe is very low value. If mean < thresh, severe is very high value.
        if direction == "Memorized > Threshold":
        ...
        for idx, row in df_misclassified.iterrows():
            ...
            # Skip if we already visualized this prompt (optional, remove this check if you want sample diversity)
            # if p_idx in processed_prompts: continue
            ...
            bad_noise = init_noises[int(s_idx)].unsqueeze(0) # [1, 4, 64, 64]
            ...
            # Pick a random index distinct from s_idx
