...
def main():
    ...
    parser.add_argument("--output_file", type=str, default="./results/calculate_condition_Jacobin_norm_CJDDT/jacobian_analysis_results.csv")
    ...
    t_init = torch.full((args.num_init_noise,), 999, device=device, dtype=torch.long)
    ...
    for idx in tqdm(indices_to_analyze, desc="Processing Checkpoints"):
        ...
        def process_prompts(prompts, condition_label, cls_label, prompt_indices):
            ...
            # Warning: prompt_embeds needs to be repeated for each noise
            # Strategy: For each prompt, we run against all 100 noises.
            
            # Encode all distinct prompts first
            # unique_prompts = list(set(prompts)) # Actually prompts list is aligned with indices, keep as is
            # Ideally encode efficiently
